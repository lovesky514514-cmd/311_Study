...
let LIB={docs:[],pages:[]},KNOWLEDGE=[],CURRENT=[],CURRENT_RECALL=null;
...
function extractAnswer(q,rows){const a=[];for(let ri=0;ri<Math.min(10,rows.length);ri++){const r=rows[ri],rankBonus=Math.max(0,20-ri*3);for(const s of sentences(r.p.text)){const sc=sentenceScore(q,s)+sourceBoost(r.p)+rankBonus;if(sc>0)a.push({s,sc,p:r.p})}}a.sort((x,y)=>y.sc-x.sc);const out=[],seen=new Set();for(const x of a){const n=norm(x.s);if(seen.has(n))continue;seen.add(n);out.push(x);if(out.length>=3)break}return out}
